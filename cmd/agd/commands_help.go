...
7) Tag incident feature root:
   agd incident-tag checkout_incident_case FT-CHECKOUT core_spec_checkout CORE-010 --reason "set exact root" --impact "AI finds target section fast"

8) Generate kit in one shot:
   agd starter-kit checkout
   agd new-project checkout --owner product-team
   agd maintenance checkout --tag-source core_spec_checkout --tag-section CORE-010
   agd incident checkout --tag-source core_spec_checkout --tag-section CORE-010
